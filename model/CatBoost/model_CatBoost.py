...
# ----------------------------
# Train Multi-output CatBoost
# ----------------------------
def train_cat_models(feature_train, target_train):
    ...
    for i in range(n_outputs):
        ...
        models.append(model)
        print(f"Trained CatBoost for output {i+1}/{n_outputs}")
    return models
...
if __name__ == "__main__":
    ...
    cat_models = train_cat_models(feature_train_norm, target_train_norm)
    predictions_norm = predict_cat(cat_models, feature_test_norm)
    ...
    # Save models
    model_path = "/mnt/storage/emon/model_library/CAT_TX11"
    ...
    for i, model in enumerate(cat_models):
        model.save_model(os.path.join(model_path, f'cat_model_output_{i}.cbm'))